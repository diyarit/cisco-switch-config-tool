...
class CiscoConfigTool:
    def __init__(self, root):
        self.root = root
        self.root.title("Cisco Configuration Tool v6.0")
        ...
        self.global_configs = {
            ...
            "gateway_ip": "",
            # Advanced Features
            "stp_mode": "pvst",  # pvst, rapid-pvst, mst
            "stp_priority": "32768",
            "vtp_mode": "transparent",  # server, client, transparent, off
            "vtp_domain": "",
            "vtp_password": "",
            "snmp_community": "",
            "snmp_location": "",
            ...
            "acl_standard": {},  # {acl_name: [rules]}
            "acl_extended": {},  # {acl_name: [rules]}
            "qos_trust": "cos",  # cos, dscp, none
            "storm_control": {
        ...
        # Ensure output starts clean and show welcome message
        self.output_text.delete("1.0", tk.END)
        self.output_text.insert(tk.END, "Cisco Switch Configuration Tool v6.0\n")
        self.output_text.insert(tk.END, "Ready to generate configurations. Select ports and configure settings.\n")
        self.output_text.insert(tk.END, "Use 'Generate Full Config' to create complete switch configuration.\n")
    ...
    def _create_main_layout(self):
        ...
        advanced_tab = ttk.Frame(self.config_notebook)
        self.config_notebook.add(advanced_tab, text="Advanced")
        self._create_advanced_features_widgets(advanced_tab)

        # Add Security Tab
        security_tab = ttk.Frame(self.config_notebook)
        self.config_notebook.add(security_tab, text="Security")
        self._create_security_widgets(security_tab)

        # Add QoS Tab
        qos_tab = ttk.Frame(self.config_notebook)
        self.config_notebook.add(qos_tab, text="QoS")
        self._create_qos_widgets(qos_tab)
    ...
    def _show_all_configurations(self):
        ...
        # Automatically detect and add missing VLANs from port configurations
        self._auto_detect_missing_vlans()
        ...
        for port_num in configured_ports:
            ...
            # Port Security
            port_security = port_config.get("port_security", {})
            if port_security.get("enabled", False):
                config_lines.append(" switchport port-security")
                if "max_mac" in port_security:
                    config_lines.append(f" switchport port-security maximum {port_security['max_mac']}")
                if "violation" in port_security:
                    config_lines.append(f" switchport port-security violation {port_security['violation']}")
                if port_security.get("sticky", False):
                    config_lines.append(" switchport port-security mac-address sticky")

            # EtherChannel
            etherchannel = port_config.get("etherchannel", {})
            if etherchannel.get("enabled", False):
                group = etherchannel.get("group", "")
                if group:
                    config_lines.append(f" channel-group {group} mode {etherchannel.get('mode', 'lacp')}")

            # Storm Control
            storm_control = port_config.get("storm_control", {})
            for storm_type in ["broadcast", "multicast", "unicast"]:
                if storm_control.get(storm_type, {}).get("enabled", False):
                    threshold = storm_control[storm_type].get("threshold", "80")
                    config_lines.append(f" storm-control {storm_type} level {threshold} {threshold}")

            # PoE Configuration
            poe = port_config.get("poe", {})
            if not poe.get("enabled", True):
                config_lines.append(" no power inline")
            elif "power_limit" in poe:
                config_lines.append(f" power inline limit {poe['power_limit']}")
            if "priority" in poe and poe["priority"] != "low":
                config_lines.append(f" power inline priority {poe['priority']}")

            # ACL Configuration
            acl = port_config.get("acl", {})
            if acl.get("inbound"):
                config_lines.append(f" ip access-group {acl['inbound']} in")
            if acl.get("outbound"):
                config_lines.append(f" ip access-group {acl['outbound']} out")
    ...
    def _show_global_configurations(self):
        ...
        # STP Configuration
        stp_mode = self.global_configs.get("stp_mode", "pvst")
        stp_priority = self.global_configs.get("stp_priority", "32768")
        if stp_mode and stp_priority:
            self.output_text.insert(tk.END, f"\n! --- STP Configuration ---\n")
            if stp_mode == "rapid-pvst":
                self.output_text.insert(tk.END, "spanning-tree mode rapid-pvst\n")
            elif stp_mode == "mst":
                self.output_text.insert(tk.END, "spanning-tree mode mst\n")
            else:
                self.output_text.insert(tk.END, "spanning-tree mode pvst\n")
            self.output_text.insert(tk.END, f"spanning-tree vlan 1 priority {stp_priority}\n")

        # VTP Configuration
        vtp_mode = self.global_configs.get("vtp_mode", "transparent")
        vtp_domain = self.global_configs.get("vtp_domain", "")
        vtp_password = self.global_configs.get("vtp_password", "")
        if vtp_mode:
            self.output_text.insert(tk.END, f"\n! --- VTP Configuration ---\n")
            if vtp_mode == "off":
                self.output_text.insert(tk.END, "vtp mode off\n")
            else:
                self.output_text.insert(tk.END, f"vtp mode {vtp_mode}\n")
                if vtp_domain:
                    self.output_text.insert(tk.END, f"vtp domain {vtp_domain}\n")
                if vtp_password:
                    self.output_text.insert(tk.END, f"vtp password {vtp_password}\n")

        # SNMP Configuration
        snmp_community = self.global_configs.get("snmp_community", "")
        snmp_location = self.global_configs.get("snmp_location", "")
        snmp_contact = self.global_configs.get("snmp_contact", "")
        if snmp_community or snmp_location or snmp_contact:
            self.output_text.insert(tk.END, f"\n! --- SNMP Configuration ---\n")
            if snmp_community:
                self.output_text.insert(tk.END, f"snmp-server community {snmp_community} RO\n")
            if snmp_location:
                self.output_text.insert(tk.END, f"snmp-server location {snmp_location}\n")
            if snmp_contact:
                self.output_text.insert(tk.END, f"snmp-server contact {snmp_contact}\n")

        # NTP Configuration
        ntp_server = self.global_configs.get("ntp_server", "")
        ntp_source = self.global_configs.get("ntp_source", "")
        if ntp_server:
            self.output_text.insert(tk.END, f"\n! --- NTP Configuration ---\n")
            self.output_text.insert(tk.END, f"ntp server {ntp_server}\n")
            if ntp_source:
                self.output_text.insert(tk.END, f"ntp source {ntp_source}\n")

        # PoE Configuration
        poe_enabled = self.global_configs.get("poe_enabled", True)
        poe_power_limit = self.global_configs.get("poe_power_limit", "15.4")
        if not poe_enabled:
            self.output_text.insert(tk.END, f"\n! --- PoE Configuration ---\n")
            self.output_text.insert(tk.END, "no power inline\n")

        # Security Features
        dhcp_snooping = self.global_configs.get("dhcp_snooping", False)
        dai_enabled = self.global_configs.get("dai_enabled", False)
        ip_source_guard = self.global_configs.get("ip_source_guard", False)
        if dhcp_snooping or dai_enabled or ip_source_guard:
            self.output_text.insert(tk.END, f"\n! --- Security Features ---\n")
            if dhcp_snooping:
                self.output_text.insert(tk.END, "ip dhcp snooping\n")
            if dai_enabled:
                self.output_text.insert(tk.END, "ip arp inspection vlan 1\n")
            if ip_source_guard:
                self.output_text.insert(tk.END, "ip verify source\n")

        # Storm Control
        storm_control = self.global_configs.get("storm_control", {})
        if storm_control.get("broadcast", {}).get("enabled", False):
            threshold = storm_control.get("broadcast", {}).get("threshold", "80")
            self.output_text.insert(tk.END, f"\n! --- Storm Control ---\n")
            self.output_text.insert(tk.END, f"storm-control broadcast level {threshold} {threshold}\n")

        # Note: QoS trust is configured per-interface, not globally

        # ACL Configuration
        acl_standard = self.global_configs.get("acl_standard", {})
        acl_extended = self.global_configs.get("acl_extended", {})
        if acl_standard or acl_extended:
            self.output_text.insert(tk.END, f"\n! --- Access Control Lists ---\n")
            for acl_name, rules in acl_standard.items():
                self.output_text.insert(tk.END, f"access-list {acl_name}\n")
                for rule in rules:
                    self.output_text.insert(tk.END, f" {rule}\n")
            for acl_name, rules in acl_extended.items():
                self.output_text.insert(tk.END, f"access-list {acl_name}\n")
                for rule in rules:
                    self.output_text.insert(tk.END, f" {rule}\n")
    ...
    def _update_port_visuals(self):
        ...
        # Note: Removed automatic configuration display to keep output clean on startup
        ...
        for vlan in new_vlans:
            try:
                ...
                if vlan_id not in self.configured_vlans:
                    ...
                    # Automatically add VLAN to global configuration
                    if "vlans" not in self.global_configs:
                        self.global_configs["vlans"] = {}
                    
                    # Generate appropriate VLAN name based on context
                    ...
                    messagebox.showinfo("New VLAN", f"New VLAN {vlan_id} ({vlan_name}) has been automatically added to the configuration.")
            except ValueError:
    ...
    def _on_tab_changed(self, event=None):
        ...
        # Tab index 0 = Port Config, 1 = Global Config, 2 = Template Editor, 3 = Advanced, 4 = Security, 5 = QoS
        ...
        elif current_tab == 4:  # Security tab
            # Load security settings into UI
            self._load_security_settings_to_ui()
        elif current_tab == 5:  # QoS tab
            # Load QoS settings into UI
            self._load_qos_settings_to_ui()

    def _create_advanced_features_widgets(self, parent_frame):
        """Create advanced features configuration widgets."""
        # STP Configuration
        stp_frame = ttk.LabelFrame(parent_frame, text="Spanning Tree Protocol (STP)")
        stp_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        
        ttk.Label(stp_frame, text="STP Mode:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        self.stp_mode_var = tk.StringVar(value="pvst")
        stp_mode_combo = ttk.Combobox(stp_frame, textvariable=self.stp_mode_var, 
                                     values=["pvst", "rapid-pvst", "mst"], state="readonly")
        stp_mode_combo.grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(stp_frame, text="Priority:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.stp_priority_var = tk.StringVar(value="32768")
        ttk.Entry(stp_frame, textvariable=self.stp_priority_var, width=15).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        # VTP Configuration
        vtp_frame = ttk.LabelFrame(parent_frame, text="VLAN Trunking Protocol (VTP)")
        vtp_frame.grid(row=1, column=0, sticky="ew", padx=5, pady=5)
        
        ttk.Label(vtp_frame, text="VTP Mode:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        self.vtp_mode_var = tk.StringVar(value="transparent")
        vtp_mode_combo = ttk.Combobox(vtp_frame, textvariable=self.vtp_mode_var,
                                     values=["server", "client", "transparent", "off"], state="readonly")
        vtp_mode_combo.grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(vtp_frame, text="Domain:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.vtp_domain_var = tk.StringVar()
        ttk.Entry(vtp_frame, textvariable=self.vtp_domain_var, width=20).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(vtp_frame, text="Password:").grid(row=2, column=0, sticky="w", padx=5, pady=2)
        self.vtp_password_var = tk.StringVar()
        ttk.Entry(vtp_frame, textvariable=self.vtp_password_var, width=20, show="*").grid(row=2, column=1, sticky="ew", padx=5, pady=2)
        
        # SNMP Configuration
        snmp_frame = ttk.LabelFrame(parent_frame, text="SNMP Configuration")
        snmp_frame.grid(row=2, column=0, sticky="ew", padx=5, pady=5)
        
        ttk.Label(snmp_frame, text="Community:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        self.snmp_community_var = tk.StringVar()
        ttk.Entry(snmp_frame, textvariable=self.snmp_community_var, width=20).grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(snmp_frame, text="Location:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.snmp_location_var = tk.StringVar()
        ttk.Entry(snmp_frame, textvariable=self.snmp_location_var, width=30).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(snmp_frame, text="Contact:").grid(row=2, column=0, sticky="w", padx=5, pady=2)
        self.snmp_contact_var = tk.StringVar()
        ttk.Entry(snmp_frame, textvariable=self.snmp_contact_var, width=30).grid(row=2, column=1, sticky="ew", padx=5, pady=2)
        
        # NTP Configuration
        ntp_frame = ttk.LabelFrame(parent_frame, text="Network Time Protocol (NTP)")
        ntp_frame.grid(row=3, column=0, sticky="ew", padx=5, pady=5)
        
        ttk.Label(ntp_frame, text="NTP Server:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        self.ntp_server_var = tk.StringVar()
        ttk.Entry(ntp_frame, textvariable=self.ntp_server_var, width=20).grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(ntp_frame, text="Source Interface:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.ntp_source_var = tk.StringVar()
        ttk.Entry(ntp_frame, textvariable=self.ntp_source_var, width=20).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        # PoE Configuration
        poe_frame = ttk.LabelFrame(parent_frame, text="Power over Ethernet (PoE)")
        poe_frame.grid(row=4, column=0, sticky="ew", padx=5, pady=5)
        
        self.poe_enabled_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(poe_frame, text="Enable PoE", variable=self.poe_enabled_var).grid(row=0, column=0, sticky="w", padx=5, pady=2)
        
        ttk.Label(poe_frame, text="Power Limit (W):").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.poe_power_limit_var = tk.StringVar(value="15.4")
        ttk.Entry(poe_frame, textvariable=self.poe_power_limit_var, width=10).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        # Apply button
        ttk.Button(parent_frame, text="Apply Advanced Settings", 
                  command=self._apply_advanced_settings).grid(row=5, column=0, pady=10)

    def _create_security_widgets(self, parent_frame):
        """Create security configuration widgets."""
        # Port Security
        port_sec_frame = ttk.LabelFrame(parent_frame, text="Port Security")
        port_sec_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        
        self.port_security_enabled_var = tk.BooleanVar()
        ttk.Checkbutton(port_sec_frame, text="Enable Port Security", 
                        variable=self.port_security_enabled_var).grid(row=0, column=0, columnspan=2, sticky="w", padx=5, pady=2)
        
        ttk.Label(port_sec_frame, text="Max MAC Addresses:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.port_security_max_mac_var = tk.StringVar(value="1")
        ttk.Entry(port_sec_frame, textvariable=self.port_security_max_mac_var, width=10).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(port_sec_frame, text="Violation Action:").grid(row=2, column=0, sticky="w", padx=5, pady=2)
        self.port_security_violation_var = tk.StringVar(value="shutdown")
        violation_combo = ttk.Combobox(port_sec_frame, textvariable=self.port_security_violation_var,
                                      values=["shutdown", "restrict", "protect"], state="readonly")
        violation_combo.grid(row=2, column=1, sticky="ew", padx=5, pady=2)
        
        self.port_security_sticky_var = tk.BooleanVar()
        ttk.Checkbutton(port_sec_frame, text="Sticky Learning", 
                       variable=self.port_security_sticky_var).grid(row=3, column=0, columnspan=2, sticky="w", padx=5, pady=2)
        
        # DHCP Snooping
        dhcp_frame = ttk.LabelFrame(parent_frame, text="DHCP Snooping")
        dhcp_frame.grid(row=1, column=0, sticky="ew", padx=5, pady=5)
        
        self.dhcp_snooping_var = tk.BooleanVar()
        ttk.Checkbutton(dhcp_frame, text="Enable DHCP Snooping", 
                       variable=self.dhcp_snooping_var).grid(row=0, column=0, columnspan=2, sticky="w", padx=5, pady=2)
        
        # Dynamic ARP Inspection
        dai_frame = ttk.LabelFrame(parent_frame, text="Dynamic ARP Inspection (DAI)")
        dai_frame.grid(row=2, column=0, sticky="ew", padx=5, pady=5)
        
        self.dai_enabled_var = tk.BooleanVar()
        ttk.Checkbutton(dai_frame, text="Enable DAI", 
                       variable=self.dai_enabled_var).grid(row=0, column=0, columnspan=2, sticky="w", padx=5, pady=2)
        
        # IP Source Guard
        ipsg_frame = ttk.LabelFrame(parent_frame, text="IP Source Guard")
        ipsg_frame.grid(row=3, column=0, sticky="ew", padx=5, pady=5)
        
        self.ipsg_enabled_var = tk.BooleanVar()
        ttk.Checkbutton(ipsg_frame, text="Enable IP Source Guard", 
                       variable=self.ipsg_enabled_var).grid(row=0, column=0, columnspan=2, sticky="w", padx=5, pady=2)
        
        # Storm Control
        storm_frame = ttk.LabelFrame(parent_frame, text="Storm Control")
        storm_frame.grid(row=4, column=0, sticky="ew", padx=5, pady=5)
        
        self.storm_broadcast_var = tk.BooleanVar()
        ttk.Checkbutton(storm_frame, text="Broadcast Storm Control", 
                       variable=self.storm_broadcast_var).grid(row=0, column=0, sticky="w", padx=5, pady=2)
        
        ttk.Label(storm_frame, text="Threshold (%):").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.storm_threshold_var = tk.StringVar(value="80")
        ttk.Entry(storm_frame, textvariable=self.storm_threshold_var, width=10).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        # Apply button
        ttk.Button(parent_frame, text="Apply Security Settings", 
                  command=self._apply_security_settings).grid(row=5, column=0, pady=10)

    def _create_qos_widgets(self, parent_frame):
        """Create QoS configuration widgets."""
        # QoS Trust
        trust_frame = ttk.LabelFrame(parent_frame, text="QoS Trust Settings")
        trust_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        
        ttk.Label(trust_frame, text="Trust Type:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        self.qos_trust_var = tk.StringVar(value="cos")
        trust_combo = ttk.Combobox(trust_frame, textvariable=self.qos_trust_var,
                                 values=["cos", "dscp", "none"], state="readonly")
        trust_combo.grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        
        # ACL Management
        acl_frame = ttk.LabelFrame(parent_frame, text="Access Control Lists")
        acl_frame.grid(row=1, column=0, sticky="ew", padx=5, pady=5)
        
        ttk.Label(acl_frame, text="Standard ACL Name:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        self.acl_standard_name_var = tk.StringVar()
        ttk.Entry(acl_frame, textvariable=self.acl_standard_name_var, width=20).grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(acl_frame, text="Extended ACL Name:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.acl_extended_name_var = tk.StringVar()
        ttk.Entry(acl_frame, textvariable=self.acl_extended_name_var, width=20).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        # ACL Rules
        ttk.Label(acl_frame, text="ACL Rules (one per line):").grid(row=2, column=0, sticky="nw", padx=5, pady=2)
        self.acl_rules_text = scrolledtext.ScrolledText(acl_frame, height=6, width=50)
        self.acl_rules_text.grid(row=3, column=0, columnspan=2, sticky="ew", padx=5, pady=2)
        
        # EtherChannel Configuration
        etherchannel_frame = ttk.LabelFrame(parent_frame, text="EtherChannel")
        etherchannel_frame.grid(row=2, column=0, sticky="ew", padx=5, pady=5)
        
        self.etherchannel_enabled_var = tk.BooleanVar()
        ttk.Checkbutton(etherchannel_frame, text="Enable EtherChannel", 
                       variable=self.etherchannel_enabled_var).grid(row=0, column=0, columnspan=2, sticky="w", padx=5, pady=2)
        
        ttk.Label(etherchannel_frame, text="Mode:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.etherchannel_mode_var = tk.StringVar(value="lacp")
        mode_combo = ttk.Combobox(etherchannel_frame, textvariable=self.etherchannel_mode_var,
                                values=["lacp", "pagp", "on"], state="readonly")
        mode_combo.grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(etherchannel_frame, text="Group Number:").grid(row=2, column=0, sticky="w", padx=5, pady=2)
        self.etherchannel_group_var = tk.StringVar()
        ttk.Entry(etherchannel_frame, textvariable=self.etherchannel_group_var, width=10).grid(row=2, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(etherchannel_frame, text="Load Balance:").grid(row=3, column=0, sticky="w", padx=5, pady=2)
        self.etherchannel_load_balance_var = tk.StringVar(value="src-dst-ip")
        load_balance_combo = ttk.Combobox(etherchannel_frame, textvariable=self.etherchannel_load_balance_var,
                                        values=["src-dst-ip", "src-dst-mac", "src-ip", "dst-ip"], state="readonly")
        load_balance_combo.grid(row=3, column=1, sticky="ew", padx=5, pady=2)
        
        # Apply button
        ttk.Button(parent_frame, text="Apply QoS Settings", 
                  command=self._apply_qos_settings).grid(row=3, column=0, pady=10)

    def _apply_advanced_settings(self):
        """Apply advanced configuration settings."""
        try:
            # Update global configs with advanced settings
            self.global_configs["stp_mode"] = self.stp_mode_var.get()
            self.global_configs["stp_priority"] = self.stp_priority_var.get()
            self.global_configs["vtp_mode"] = self.vtp_mode_var.get()
            self.global_configs["vtp_domain"] = self.vtp_domain_var.get()
            self.global_configs["vtp_password"] = self.vtp_password_var.get()
            self.global_configs["snmp_community"] = self.snmp_community_var.get()
            self.global_configs["snmp_location"] = self.snmp_location_var.get()
            self.global_configs["snmp_contact"] = self.snmp_contact_var.get()
            self.global_configs["ntp_server"] = self.ntp_server_var.get()
            self.global_configs["ntp_source"] = self.ntp_source_var.get()
            self.global_configs["poe_enabled"] = self.poe_enabled_var.get()
            self.global_configs["poe_power_limit"] = self.poe_power_limit_var.get()
            
            self._save_global_configs()
            self.update_status("Advanced settings applied successfully.")
    ...
    def _apply_security_settings(self):
        ...
        try:
            # Update global configs with security settings
            self.global_configs["dhcp_snooping"] = self.dhcp_snooping_var.get()
            self.global_configs["dai_enabled"] = self.dai_enabled_var.get()
            self.global_configs["ip_source_guard"] = self.ipsg_enabled_var.get()
            
            # Update storm control settings
            self.global_configs["storm_control"]["broadcast"]["enabled"] = self.storm_broadcast_var.get()
            self.global_configs["storm_control"]["broadcast"]["threshold"] = self.storm_threshold_var.get()
            
            self._save_global_configs()
            self.update_status("Security settings applied successfully.")
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to apply security settings: {str(e)}")

    def _apply_qos_settings(self):
        """Apply QoS configuration settings."""
        try:
            # Update global configs with QoS settings
            self.global_configs["qos_trust"] = self.qos_trust_var.get()
            
            # Handle ACL rules
            acl_rules = self.acl_rules_text.get("1.0", tk.END).strip().split('\n')
            acl_rules = [rule.strip() for rule in acl_rules if rule.strip()]
            
            if self.acl_standard_name_var.get():
                self.global_configs["acl_standard"][self.acl_standard_name_var.get()] = acl_rules
            
            if self.acl_extended_name_var.get():
                self.global_configs["acl_extended"][self.acl_extended_name_var.get()] = acl_rules
            
            self._save_global_configs()
            self.update_status("QoS settings applied successfully.")
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to apply QoS settings: {str(e)}")

    def _load_advanced_settings_to_ui(self):
        """Load advanced settings from global_configs into UI."""
        try:
            self.stp_mode_var.set(self.global_configs.get("stp_mode", "pvst"))
            self.stp_priority_var.set(self.global_configs.get("stp_priority", "32768"))
            self.vtp_mode_var.set(self.global_configs.get("vtp_mode", "transparent"))
            self.vtp_domain_var.set(self.global_configs.get("vtp_domain", ""))
            self.vtp_password_var.set(self.global_configs.get("vtp_password", ""))
            self.snmp_community_var.set(self.global_configs.get("snmp_community", ""))
            self.snmp_location_var.set(self.global_configs.get("snmp_location", ""))
            self.snmp_contact_var.set(self.global_configs.get("snmp_contact", ""))
            self.ntp_server_var.set(self.global_configs.get("ntp_server", ""))
            self.ntp_source_var.set(self.global_configs.get("ntp_source", ""))
            self.poe_enabled_var.set(self.global_configs.get("poe_enabled", True))
            self.poe_power_limit_var.set(self.global_configs.get("poe_power_limit", "15.4"))
        except Exception as e:
            print(f"Error loading advanced settings: {e}")

    def _load_security_settings_to_ui(self):
        """Load security settings from global_configs into UI."""
        try:
            self.dhcp_snooping_var.set(self.global_configs.get("dhcp_snooping", False))
            self.dai_enabled_var.set(self.global_configs.get("dai_enabled", False))
            self.ipsg_enabled_var.set(self.global_configs.get("ip_source_guard", False))
            
            storm_control = self.global_configs.get("storm_control", {})
            self.storm_broadcast_var.set(storm_control.get("broadcast", {}).get("enabled", False))
            self.storm_threshold_var.set(storm_control.get("broadcast", {}).get("threshold", "80"))
        except Exception as e:
            print(f"Error loading security settings: {e}")

    def _load_qos_settings_to_ui(self):
        """Load QoS settings from global_configs into UI."""
        try:
            self.qos_trust_var.set(self.global_configs.get("qos_trust", "cos"))
            ...
            if acl_standard:
                acl_name = list(acl_standard.keys())[0]
                self.acl_standard_name_var.set(acl_name)
                rules = acl_standard[acl_name]
                self.acl_rules_text.delete("1.0", tk.END)
                self.acl_rules_text.insert("1.0", "\n".join(rules))
            
            if acl_extended:
                acl_name = list(acl_extended.keys())[0]
                self.acl_extended_name_var.set(acl_name)
                if not acl_standard:  # Only set rules if no standard ACL
                    rules = acl_extended[acl_name]
                    self.acl_rules_text.delete("1.0", tk.END)
                    self.acl_rules_text.insert("1.0", "\n".join(rules))
        except Exception as e:
            print(f"Error loading QoS settings: {e}")

    def _auto_detect_missing_vlans(self):
        """Automatically detect and add VLANs that are referenced in port configurations but not in global config."""
        detected_vlans = set()
        
        # Scan all port configurations for VLAN references
        for port_num, port_config in self.port_configs.items():
            # Check data VLAN
            if "data_vlan" in port_config:
                try:
                    vlan_id = int(port_config["data_vlan"])
            ...
            if "voice_vlan" in port_config:
                try:
                    vlan_id = int(port_config["voice_vlan"])
                    detected_vlans.add(vlan_id)
                except ValueError:
                    pass
            ...
            if "native_vlan" in port_config:
                try:
                    vlan_id = int(port_config["native_vlan"])
                    detected_vlans.add(vlan_id)
                except ValueError:
                    pass
        ...
        # Always ensure VLAN 1 is included (default VLAN)
        detected_vlans.add(1)
        
        # Add missing VLANs to global configuration
        if "vlans" not in self.global_configs:
            self.global_configs["vlans"] = {}
        
        added_vlans = []
        for vlan_id in detected_vlans:
            if vlan_id not in self.configured_vlans:
                self.configured_vlans.add(vlan_id)
                
                # Generate appropriate VLAN name
                vlan_name = f"VLAN_{vlan_id}"
                if vlan_id == 1:
                    vlan_name = "Default"
                elif vlan_id == 100:
                ...
                self.global_configs["vlans"][str(vlan_id)] = vlan_name
                added_vlans.append(f"{vlan_id} ({vlan_name})")
        
        # Save global configs if any VLANs were added
        if added_vlans:
            self._save_global_configs()
            self.update_status(f"Auto-detected and added VLANs: {', '.join(added_vlans)}")
